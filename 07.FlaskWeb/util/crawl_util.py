...
def get_bestseller():
    ...
    for li in lis:
        ...
        href = li.select_one('.coverImage > label > a')['href']     # 자식 셀렉터 , 들어가는 사이트 링크
        src = li.select_one('.coverImage img')['src']               # 자손 셀렉터 a 태그가 하나임으로...
        ...
        data.append({'순위':rank, '제목':title, '저자':author, '출판사':company, 
                     '가격':f'{price:7,d}', 'href':base_url+href, 'src':src})               # 가격은 10만원대까지만 
